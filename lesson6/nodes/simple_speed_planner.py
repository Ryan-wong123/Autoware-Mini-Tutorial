...
class SimpleSpeedPlanner:
    ...
    def collision_points_and_path_callback(self, collision_points_msg, local_path_msg):
        try:
            ...
            closest_collision_idx = np.argmin(calculated_target_velocities)
            target_velocity = calculated_target_velocities[closest_collision_idx]

            for wp in local_path_msg.waypoints:
                wp.speed = min(target_velocity, wp.speed)

            target_object_distance = collision_point_distances[closest_collision_idx] - self.distance_to_car_front
            target_object_speed = collision_point_speeds[closest_collision_idx]
            collision_point_braking_distance = collision_point_braking_distances[closest_collision_idx]
            collision_point_category = collision_points[closest_collision_idx]["category"]
            stopping_point_distance = collision_point_distances[closest_collision_idx] - collision_point_braking_distance
